...
function normalizeCodeJson(value) {
  ...
  return Object.entries(value).reduce((codes, [codeName, codeValue]) => {
...
async function handleSavedCodingFile(file) {
  ...
  try {
    const contents = await file.text();
    const savedCodes = normalizeCodeJson(JSON.parse(contents));
...
function renderReader() {
  ...
  const blocks = splitTranscript(doc.text)
    .map((block) => renderBlock(block, state.search))
    .join("");
...
function renderBlock(block, query) {
  const body = highlight(escapeHtml(block.text), query);
